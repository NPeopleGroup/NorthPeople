$(function() {
	...
	function check_part_input() {
		...
		if ($("input[name='username']").val() == "") {
			checkpassed = false;
			var a = $("input[name='username']").nextAll('span').eq(0);
			a.text("请输入您的账号");
		} else if (!check_username($("input[name='username']").val())) {
			checkpassed = false;
			var a = $("input[name='username']").nextAll('span').eq(0);
			a.text("格式不正确");
		} else {
		...
		if ($("input[name='password']").val() == "") {
			checkpassed = false;
			var a = $("input[name='password']").nextAll('span').eq(0);
			a.text("请输入密码");
		} else if (!check_password($("input[name='password']").val())) {
			checkpassed = false;
			var a = $("input[name='password']").nextAll('span').eq(0);
			a.text("格式不正确");
		} else {
